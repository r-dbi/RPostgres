...
#' Set `drv = Postgres()` to connect to a PostgreSQL(-ish) database.
#' Use `drv = Redshift()` instead to connect to an AWS Redshift cluster.
#'
#' Manually disconnecting a connection is not necessary with \pkg{RPostgres},
#' but still recommended;
#' if you delete the object containing the connection, it will be automatically disconnected during the next GC with a warning.
#'
#' @param drv [DBI::DBIDriver-class].
#'   Use [Postgres()] to connect to a PostgreSQL(-ish) database or [Redshift()] to connect to an AWS Redshift cluster.
#'   Use an existing [DBI::DBIConnection-class] object to clone an existing connection.
#' @param dbname Database name.
#'   If `NULL`, defaults to the user name.
#'   Note that this argument can only contain the database name,
#'   it will not be parsed as a connection string
#'   (internally, `expand_dbname` is set to `false` in the call to [`PQconnectdbParams()`](https://www.postgresql.org/docs/current/libpq-connect.html)).
#' @param user,password User name and password.
#'   If `NULL`, will be retrieved from `PGUSER` and `PGPASSWORD` envvars, or from the appropriate line in `~/.pgpass`.
...
#' @param service Name of service to connect as.
#'   If `NULL`, will be ignored.
#'   Otherwise, connection parameters will be loaded from the pg_service.conf file and used.
#'   See <https://www.postgresql.org/docs/current/libpq-pgservice.html> for details on this file and syntax.
...
#' @param check_interrupts Should user interrupts be checked during the query execution (before first row of data is available)?
#'   Setting to `TRUE` allows interruption of queries running too long.
#' @param timezone Sets the timezone for the connection.
#'   The default is `"UTC"`.
